...
import { formatDate, formatRelative, cn } from "@/lib/utils/index";
import type {
  ApiKey,
  ApiKeyCreated,
        ...
        <div className="space-y-4 p-6">
          <div className="rounded-lg bg-amber-50 border border-amber-200 p-4">
            <p className="text-sm font-medium text-amber-800">
              ⚠️ Simpan key ini sekarang
            </p>
            <p className="text-xs text-amber-700 mt-1">
              Raw key hanya ditampilkan sekali. Setelah ditutup, tidak bisa
              dilihat lagi.
            </p>
          </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500">
          {apiKeys?.length ?? 0} API key aktif
        </p>
        <button
              ...
              <div key={key.id} className="flex items-center gap-4 px-5 py-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    ...
                    {key.last_used_at &&
                      ` · Terakhir digunakan ${formatRelative(key.last_used_at)}`}
                    {key.expires_at &&
                      ` · Kadaluarsa ${formatDate(key.expires_at)}`}
                  </p>
                </div>
                <ConfirmDialog
                  trigger={
...
function WebhooksTab() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500">
          {webhooks?.length ?? 0} webhook aktif
        </p>
        <button
          onClick={() => setShowCreate(true)}
          className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 transition-colors"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1.5">
                    {wh.events.slice(0, 4).map((event) => (
                      <span
                        key={event}
                        className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-mono text-slate-600"
                      >
                        {event}
                      </span>
                    ))}
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    ...
                    <span
                      className={
                        wh.is_active ? "text-emerald-600" : "text-slate-400"
                      }
                    >
