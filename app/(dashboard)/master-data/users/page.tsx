...
import { tenantUserApi, roleApi, type TenantUser, type InviteUser } from '@/lib/api/modules/masterdata'
...
function InviteModal({ onClose }: { onClose: () => void }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (form.password !== confirmPass) { toast.error('Konfirmasi password tidak cocok'); return }
    invite.mutate()
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-xl bg-white shadow-xl">
        <div className="border-b border-slate-200 px-6 py-4">
          <h2 className="text-base font-semibold text-slate-900">Tambah Pengguna</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-6">
          ...
          <div>
            ...
            <input type="email" value={form.email}
              ...
              required placeholder="budi@bisnis.com" className={inp} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="password" value={form.password}
                ...
                required minLength={8} placeholder="••••••••" className={inp} />
            </div>
            <div>
          ...
          <div className="rounded-lg bg-blue-50 border border-blue-200 p-3 text-xs text-blue-700">
            Pengguna dapat login menggunakan email dan password yang ditetapkan di atas.
          </div>
...
export default function UsersPage() {
  ...
  const deactivate = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tenant-users'] }); toast.success('Pengguna dinonaktifkan') },
    onError: (e: Error) => toast.error(e.message),
  })
