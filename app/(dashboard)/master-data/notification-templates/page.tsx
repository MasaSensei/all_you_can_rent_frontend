...
const VARIABLES = [
  '{{customer_name}}', '{{booking_number}}', '{{invoice_number}}',
  '{{amount}}', '{{start_date}}', '{{end_date}}', '{{asset_name}}',
]
        ...
        >
          ...
          <div>
            ...
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
              required placeholder="Booking Confirmed - WhatsApp" className={inp} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700">Event Trigger *</label>
              <select value={form.event_trigger}
                onChange={(e) => setForm({ ...form, event_trigger: e.target.value })} className={inp}>
                {EVENT_TRIGGERS.map((e) => <option key={e.value} value={e.value}>{e.label}</option>)}
              </select>
            </div>
          </div>
          ...
          {form.channel === 'email' && (
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700">Subject Email *</label>
              <input value={form.subject ?? ''}
                onChange={(e) => setForm({ ...form, subject: e.target.value })}
                required={form.channel === 'email'}
                placeholder="Booking {{booking_number}} Telah Dikonfirmasi" className={inp} />
            </div>
          )}
          ...
          <div>
            <div className="mb-1.5 flex items-center gap-2">
              ...
              <Info className="h-3.5 w-3.5 text-slate-400" title="Gunakan variabel di bawah" />
            </div>
...
export default function NotificationTemplatesPage() {
  ...
  const remove = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['notif-templates'] }); toast.success('Template dihapus') },
    onError: (e: Error) => toast.error(e.message),
  })
  ...
  const columns: Column<NotifTemplate>[] = [
      ...
      cell: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {row.subject && (
              <p className="text-xs text-slate-400 truncate max-w-xs">{row.subject}</p>
            )}
          </div>
        </div>
      ),
    },
    {
      ...
      header: 'Channel',
      cell: (row) => (
        <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${CHANNEL_BADGE[row.channel] ?? 'bg-slate-100 text-slate-600'}`}>
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Master Data' }, { label: 'Template Notifikasi' }]}
        actions={
      ...
      <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">
        Gunakan variabel <code className="font-mono">{'{{nama_variabel}}'}</code> dalam isi pesan.
        Sistem akan menggantinya secara otomatis saat mengirim notifikasi.
      </div>
