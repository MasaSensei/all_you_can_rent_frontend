...
const RULE_LABELS: Record<string, string> = {
  flat: 'Flat', per_day: 'Per Hari', per_hour: 'Per Jam', per_week: 'Per Minggu',
}
...
function PricingRuleModal({
  ...
  const create = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['pricing-rules'] }); toast.success('Aturan harga ditambahkan'); onClose() },
    onError: (e: Error) => toast.error(e.message),
  })

  const update = useMutation({
    mutationFn: () => pricingRuleApi.update(initial!.id!, {
      ...form,
      value: Number(form.value),
    }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['pricing-rules'] }); toast.success('Aturan harga diperbarui'); onClose() },
    onError: (e: Error) => toast.error(e.message),
  })
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-xl bg-white shadow-xl">
        ...
        <form
          onSubmit={(e) => { e.preventDefault(); initial?.id ? update.mutate() : create.mutate() }}
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                required placeholder="Tarif Harian Standard" className={inp} />
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700">Tipe *</label>
              <select value={form.rule_type} onChange={(e) => setForm({ ...form, rule_type: e.target.value })} className={inp}>
          ...
          <div>
            ...
            <input type="number" min={0} value={form.value}
              ...
              required placeholder="150000" className={inp} />
          </div>
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="mb-1.5 block text-xs font-medium text-slate-600">Asset ID (spesifik)</label>
                <input value={form.asset_id} onChange={(e) => setForm({ ...form, asset_id: e.target.value })}
                  placeholder="UUID asset..." className={inp} />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" min={1} value={form.min_duration}
                onChange={(e) => setForm({ ...form, min_duration: e.target.value })}
                placeholder="Kosongkan jika tidak ada" className={inp} />
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700">
                Maks. Durasi (hari)
              </label>
              <input type="number" min={1} value={form.max_duration}
...
export default function PricingRulesPage() {
  ...
  const remove = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['pricing-rules'] }); toast.success('Aturan dihapus') },
    onError: (e: Error) => toast.error(e.message),
  })
  ...
  const columns: Column<PricingRule>[] = [
      ...
      cell: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">{RULE_LABELS[row.rule_type] ?? row.rule_type}</p>
          </div>
        </div>
      ),
    },
    {
      ...
      align: 'right',
      cell: (row) => (
        <span className="text-sm font-semibold text-slate-800">{formatCurrency(row.value)}</span>
      ),
    },
    {
      key: 'scope',
      ...
      cell: (row) => (
        <span className="text-sm text-slate-600">
          {row.asset_id ? 'Aset spesifik' : row.category_id ? 'Kategori' : 'Semua aset'}
        </span>
      ),
    },
    {
